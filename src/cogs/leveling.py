...
class Leveling(commands.Cog):
    ...
    async def on_message(self, message: discord.Message):
        ...
        if not user_info:
            ...
            await con.execute(create_user_query, author_id, message.author.name, 0, 0)
        else:
            ...
            if final_xp > levels[user_info["level"] + 1][0]:
                ...
                if levels[final_level + 1][1] is not None:
                    ...
                    await channel.send(
                        f"Congratulations, {message.author}! You just advanced to level {final_level}, enjoy your new fancy role - {role}"
                    )

                await channel.send(
                    f"Congratulations, {message.author}! You just advanced to level {final_level}"
                )

            give_xp_query = """UPDATE users SET xp=$1, level=$2 WHERE discord_id=$3"""
            await con.execute(
                give_xp_query, final_xp, final_level, user_info["discord_id"]
            )
