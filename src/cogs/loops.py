...
class Loops(commands.Cog):
    ...
    async def refreshembed(self):
        ...
        try:
            ...
            for curr_proj, x in enumerate(projects):
                ...
                project_embed.set_thumbnail(x.thumbnail if x.thumbnail else 'https://nekyou.mangadex.com/wp-content/uploads/sites/83/2019/06/About-Nekyou.png')
                ...
                if len(list_in_progress_project) != 0:
                    ...
                    for lis in lists:
                        if first:
                            ...
                            project_embed.add_field(name="Chapters in Progress", value="" + c, inline=False)
                            first = False
                        else:
                            c = " ".join(b for b in lis)
                ...
                if len(list_done_project) != 0:
                    ...
                    for lis in lists:
                        if first:
                            c = " ".join(b for b in lis)
                            project_embed.add_field(name="Chapters ready for release", value="" + c, inline=False)
                            first = False
                        else:
                            c = " ".join(b for b in lis)
                            project_embed.add_field(name="\u200b", value="" + c, inline=False)
    ...
    async def refresherror(self, e):
        ch = await self.bot.fetch_channel(self.bot.config["server"]["channels"]["board"])
